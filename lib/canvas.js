...
class Canvas {
  ...
  drawPath(path: CanvasPath, resolve: (this) => void, lastTick: ?number): void {
    if (!lastTick) {
      console.log('first draw requset');
      window.requestAnimationFrame(() => this.drawPath(path, resolve, Date.now()));
      return;
    }

    this.nextFrame(60, lastTick || Date.now()).then(delta => {
      console.log('at next frame: ', delta);
      try {
        this.startLoop();
  ...
  nextFrame(fps: number, lastTick: number): Promise<number> {
    ...
    return new Promise((resolve) => {
      window.requestAnimationFrame(() => {
        this.nextFrame(fps, lastTick).then(elapsed => resolve(elapsed));
      });
    })
  }
}
...
class CanvasPath {
  ...
  draw(context: CanvasRenderingContext2D, delta: number): Promise<boolean> {
    ...
    this.components.forEach((component, index) => {
      ...
      console.log('calculating target position: ', lengthFromLast, lengthDrawn, lengthToDraw);
      ...
      console.log('updating angle, etc');
    ...
    this.drawTurtle(context, lastHeading, lastPosition);
    return Promise.resolve(lengthDrawn >= this.length);
  }

  drawTurtle(context: CanvasRenderingContext2D, heading: number, lastPosition: Position) {
    const position = Geometry.translate(lastPosition, this.x, -this.y);
