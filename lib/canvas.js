...
class Canvas {
  ...
  async draw(input: PathOrInstructions, { x, y, speed, previousPaths }: CanvasDrawOptions): Promise<CanvasPath> {
    ...
    do {
      ...
      if (previousPaths) {
        previousPaths.forEach(previousPath => this.drawPath(previousPath, turtle, 0));
      }
    } while (!await this.drawPath(canvasPath, turtle, Date.now()));

    return canvasPath;
  }

  async drawPath(path: CanvasPath, turtle: CanvasTurtle, lastTick: number): Promise<boolean> {
    const delta = await Animator.nextFrame(60, lastTick);
    return path.draw(this.context, turtle, delta);
  }
...
class CanvasPath {
  ...
  draw(context: CanvasRenderingContext2D, turtle: ?CanvasTurtle, delta: number): boolean {
    this.elapsed += delta;
    ...
    return lengthDrawn >= this.length;
  }
