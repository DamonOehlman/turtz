...
class Canvas {
  ...
  async draw(input: PathOrInstructions, { x, y, speed, previousPaths }: CanvasDrawOptions): Promise<CanvasPath> {
    ...
    await this.drawPath(canvasPath, turtle);
    return canvasPath;
  }

  async drawPath(path: CanvasPath, turtle: CanvasTurtle, lastTick: ?number): Promise<void> {
    if (lastTick == null) {
      await Animator.nextAnimationFrame();
      return this.drawPath(path, turtle, Date.now());
    }

    const delta = await Animator.nextFrame(60, lastTick);
    Canvas.reset(this.context, this.origin);

    const done = await path.draw(this.context, turtle, delta);
    if (!done) {
      await Animator.nextAnimationFrame();
      return this.drawPath(path, turtle, Date.now());
    }
