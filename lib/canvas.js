...
class Canvas {
  ...
  drawPath(path: CanvasPath, resolve: (this) => void, startTick: ?number): void {
    if (!startTick) {
    ...
    try {
      this.startLoop();
      path.draw(this.context, Date.now() - startTick).then(done => {
        if (!done) {
          window.requestAnimationFrame(() => this.drawPath(path, resolve, startTick || Date.now()));
          return;
        }
