...
class CanvasDrawingSurface {
  ...
  draw(input: PathOrInstructions, x: number = 0, y: number = 0, speed: number = DEFAULT_SPEED): ?SurfaceDrawResult {
    const path = Path.ensurePath(input);

    // calcualate the length to draw for a 2s animation
    const totalTime = Math.floor(path.length / speed) * 1000;
    const lengthToDraw = path.length * (this.elapsed / totalTime);

    // determine the actual input origin
    // this is the origin.x + x, and the origin.y - y
    // as we want the negative y values to appear below the horizontal axis
    const inputOrigin = Geometry.translate(this.origin, x, -y);
    ...
    this.context.moveTo(inputOrigin[0], inputOrigin[1]);
    ...
    path.components.forEach((component, index) => {
      ...
      switch (component.type) {
        case 'M':
          this.context.moveTo(
            inputOrigin[0] + targetPosition[0],
            inputOrigin[1] + targetPosition[1]
          );
          break;
    ...
    if (lastPosition) {
      this.drawTurtle(lastHeading, Geometry.translate(lastPosition, x, -y));
    }
